...
$script:Config = @{
    ...
    RepoRoot           = "https://raw.githubusercontent.com/YourUser/your-repo"   # Your own repo for future updates
...
function Set-Editor {
    if (Get-Variable -Name "EDITOR_Override" -ErrorAction SilentlyContinue) {
        ...
        $EDITOR = switch -Regex ($true) {
            { Test-CommandExists nvim }   { 'nvim' }
            { Test-CommandExists pvim }   { 'pvim' }
            { Test-CommandExists vim }    { 'vim' }
            { Test-CommandExists vi }     { 'vi' }
            { Test-CommandExists code }   { 'code' }
            { Test-CommandExists codium } { 'codium' }
            { Test-CommandExists notepad++ } { 'notepad++' }
            { Test-CommandExists sublime_text } { 'sublime_text' }
            default { 'notepad' }
        }
...
function Show-SetupWizard {
    ...
    Write-Host "   How often should the profile check for updates?" -ForegroundColor Gray
    Write-Host "   Options: -1 (always), 1 (daily), 7 (weekly), 30 (monthly)" -ForegroundColor Gray
    $updateResponse = Read-Host "   Enter days between updates (default: 7)"
    if ($updateResponse -match '^\d+$' -or $updateResponse -eq '-1') {
        $script:Config.UpdateIntervalDays = [int]$updateResponse
    }
    Write-Host "   Update interval set to: $($script:Config.UpdateIntervalDays) days" -ForegroundColor Green
    Write-Host ""

    # Repo Root (for advanced users)
    Write-Host "3. Update Repository (Advanced)" -ForegroundColor Cyan
    Write-Host "   Current repository: $($script:Config.RepoRoot)" -ForegroundColor Gray
    $repoResponse = Read-Host "   Enter new repository URL (or press Enter to keep current)"
    if ($repoResponse -ne '') {
        $script:Config.RepoRoot = $repoResponse
        Write-Host "   Repository updated to: $($script:Config.RepoRoot)" -ForegroundColor Green
    } else {
        Write-Host "   Repository unchanged." -ForegroundColor Yellow
    }
    Write-Host ""
